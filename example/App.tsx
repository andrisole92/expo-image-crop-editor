...
export default function App() {
	...
	return (
		<View style={styles.container}>
			<View style={[styles.imageRow, { flexDirection: isLandscape ? 'row' : 'column' }]}>
				<Image style={styles.image} source={{ uri: imageUri }} />
				<Image style={[styles.image, { backgroundColor: '#333' }]} source={{ uri: croppedUri }} />
			</View>
			<View style={styles.buttonRow}>
				<Button title='Select Photo' onPress={() => selectPhoto()} />
				<Button title={'Aspect Lock: ' + aspectLock} onPress={() => setAspectLock(!aspectLock)} />
			</View>
...
const styles = StyleSheet.create({
	...
	image: {
		...
		backgroundColor: '#ccc',
		margin: '3%',
		...(Platform.OS === 'web' ? { width: 300, height: 300 } : { flex: 1, aspectRatio: 1 }),
	},
	buttonRow: {
		width: '100%',
		minHeight: 100,
